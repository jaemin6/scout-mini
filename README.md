...
# 🎥 Webcam 연결 시 환경 셋팅 (ROS2 Humble) </summary>

## 🧩 1️⃣ 필요 패키지 설치
```bash
sudo apt install ros-humble-v4l2-camera                 # 기본 웹캠 드라이버 노드
sudo apt install ros-humble-rqt-image-view              # rqt 이미지 확인 도구
sudo apt install ros-humble-image-transport-plugins     # 이미지 토픽 변환 플러그인
```
## 🚀 2️⃣ 실행 방법
```
웹캠 노드 실행
...
ros2 topic list
```
→ /image_raw, /camera_info 토픽이 보이면 정상 작동
...
rqt_image_view
```
좌측 상단 드롭다운 메뉴에서 **/image_raw** 선택

화면에 웹캠 영상이 뜨면 성공! 🎉

⚠️ 만약 rqt_image_view 명령어를 찾지 못할 경우:
```
...
# 🎥 웹캠 영상 확인 방법 비교 (라즈베리 환경 기준) </summary> 
...
현재 ROS2 기반 자율주행 + SLAM + Nav2 + ArUco 마커 인식 시스템에서  
라즈베리파이에 **웹캠 영상 확인 기능**을 추가하는 방법은 크게 두 가지가 있습니다. 👇

| 구분 | 방법 ① | 방법 ② |
|------|---------|---------|
| 이름 | **MJPEG Streamer 방식 (비 ROS)** | **ROS2 Webcam Topic 방식** |
| 핵심 아이디어 | 웹캠 → HTTP 스트림 송출 | 웹캠 → ROS 이미지 토픽(`/image_raw`) 퍼블리시 |
| 대표 명령어 | `mjpg_streamer -i "input_uvc.so ..." -o "output_http.so ..."` | `ros2 run v4l2_camera v4l2_camera_node` |

---

## ⚙️ ① MJPEG Streamer 방식 (비 ROS 방식)

### 📌 구조
[라즈베리] 📷 USB Webcam ─▶ MJPEG Streamer ─▶ http://라즈베리IP:8080

↓
[로컬 PC] 🖥️ 브라우저로 실시간 영상 확인


### ✅ 장점
| 항목 | 설명 |
|------|------|
| 🧠 **CPU 부하 최소화** | 영상 전송이 단순 HTTP 스트림이므로 ROS 메시지 직렬화 없음 (CPU 사용량 약 10~15%) |
| ⚡ **실시간성 우수** | MJPEG 압축으로 빠르고 딜레이 적음 |
| 🧩 **ROS와 완전히 독립** | SLAM, Nav2, LiDAR 등과 자원 충돌 없음 |
| 🌐 **브라우저 기반 접근** | 어디서든 접속 가능 (PC, 스마트폰 등) |
| 🔧 **설정 간단** | 설치 후 단일 명령으로 즉시 송출 가능 |

### ⚠️ 단점
| 항목 | 설명 |
|------|------|
| ❌ **ROS 토픽 아님** | `/image_raw` 같은 토픽으로 접근 불가 |
| ⚙️ **ArUco 인식 불가 (직접은)** | ROS2 노드가 영상을 받지 못하므로, ArUco 인식하려면 중간 브릿지 노드가 필요 |
| 📡 **HTTP 스트림만 제공** | SLAM 시각화와 영상 통합이 어려움 (RViz 불가) |

### 💡 활용 추천
- 단순히 “라즈베리 캠이 잘 보이나?” 확인할 때  
- ROS 부하를 최소화하고 영상만 보고 싶을 때  
- ArUco 인식은 라즈베리 내부에서만 처리하고, 영상은 웹에서 보기 원할 때  

---

## ⚙️ ② ROS2 Webcam Topic 방식

### 📌 구조
[라즈베리] 📷 Webcam ─▶ v4l2_camera_node ─▶ /image_raw (ROS Topic)
↓
[RViz2 / rqt_image_view] 영상 확인


### ✅ 장점
| 항목 | 설명 |
|------|------|
| 🔗 **ROS 완전 통합** | `/camera/image_raw`를 그대로 ArUco, SLAM, Navigation 등에 활용 가능 |
| 🧩 **ArUco 인식 직접 연결 가능** | `ros2_aruco` 노드가 영상 토픽을 바로 구독 가능 |
| 🧠 **RViz에서 함께 시각화** | 맵, 로봇, 마커, 영상 모두 한 화면에서 확인 가능 |
| 🔄 **Topic 기반 확장성** | 다양한 ROS 툴(rqt, image_pipeline 등)과 호환 |

### ⚠️ 단점
| 항목 | 설명 |
|------|------|
| 🔥 **CPU 부하 매우 높음** | ROS 이미지 직렬화 + 압축 해제 + 송출로 부하 큼 (특히 라즈베리에서) |
| 🕒 **프레임 드롭 발생** | 10~20fps 이상이면 딜레이/끊김 심함 |
| 🧱 **자원 충돌 위험** | SLAM/Nav2와 동시에 실행 시 메모리 부족 가능 |
| ⚙️ **설정 복잡도 약간 있음** | 해상도, 프레임, QoS 세부 조정 필요 |

### 💡 활용 추천
- ArUco 마커 인식을 ROS 내에서 직접 처리하고 싶을 때  
- 라즈베리 대신 **데스크탑/서버에서 영상처리를 담당할 때**  
- 영상 + SLAM 맵을 RViz 한 화면에서 보고 싶을 때  

---

## ⚖️ 비교 요약 표

| 항목 | MJPEG Streamer | ROS2 Webcam Node |
|------|----------------|------------------|
| **CPU 부하** | 🔻 매우 낮음 (10~15%) | 🔺 높음 (60~90%) |
| **실시간성** | ✅ 빠름 (HTTP 스트림) | ⚠️ 토픽 직렬화로 약간 느림 |
| **ROS 통합성** | ❌ 없음 | ✅ 완전 통합 |
| **ArUco 인식** | ❌ 직접 불가 (브릿지 필요) | ✅ 바로 가능 |
| **RViz 표시** | ❌ 불가능 | ✅ 가능 |
| **설치 위치** | 라즈베리 | 라즈베리 (또는 원격 PC) |
| **접속 방식** | 웹브라우저 | ROS2 Topic |
| **적합한 용도** | 영상 모니터링용 (저부하) | 인식/처리용 (고부하) |
| **추천 대상** | 🧩 SLAM + Nav2 동시 구동 환경 | ⚙️ 영상 중심 프로젝트 |

---

## 💬 결론 요약

> 🔹 **성능(부하) 우선** → **MJPEG Streamer** 🥇  
> 라즈베리에 부담이 거의 없고, 단순히 영상만 모니터링할 때 최고 효율.

> 🔹 **ROS 통합 & ArUco 연동 우선** → **ROS2 Webcam Node** 🧠  
> ROS 환경 내부에서 ArUco 마커를 직접 인식하거나, 영상 데이터를 활용할 때 필수.

---

📌 **추천 조합 (현 프로젝트 기준)**  
라즈베리에서는 MJPEG Streamer로 “영상만” 송출하고,  
Aruco 인식은 내부에서 Pose만 퍼블리시 → SLAM/Nav2와 결합.  
즉,

> “영상은 웹에서, 데이터는 ROS에서.”

이 구조가 가장 효율적이라 일단은 판단
</details>
...
# 🤖 Nav2까지의 최종 실행 순서 </summary>
```
> 본 문서는 **Scout Mini + RPLidar + Webcam + Nav2 자율주행 프로젝트**의  
> 실제 실행 순서를 정리한 가이드입니다.  
> SSH 원격(라즈베리)과 로컬 PC 환경을 구분하여 단계별로 실행합니다.
```
---
...
# 3️⃣ SLAM Toolbox 실행 (지도 작성 시에만)   slam 완료 후 nav2 실행 시 생략 가능
ros2 launch slam_toolbox online_async_launch.py

# 4️⃣ 웹캠 영상 노드 실행
ros2 run v4l2_camera v4l2_camera_node

# 4-1️⃣ 웹캠 해상도 고정 (640x480)
4-1. ros2 run v4l2_camera v4l2_camera_node --ros-args -p image_size:="[640, 480]"                                                            # rqt_image_view or rviz2 에서 확인 시
4-2. mjpg_streamer -i "input_uvc.so -d /dev/video0 -r 640x480 -f 10" -o "output_http.so -w /usr/local/share/mjpg-streamer/www -p 8080"       # local web으로 cam 확인 시

# 5️⃣ROS2 토픽(/image_raw)을 구독, 웹 브라우저가 확인할 수 있는 형태로 변환해주는 브리지 노드
ros2 run mjpeg_server mjpeg_server_node --ros-args -p image_topic:=/image_raw

# 6️⃣ 수동 조작(Teleop) - SLAM 시 수동 주행용
ros2 run teleop_twist_keyboard teleop_twist_keyboard

# 7️⃣ Nav2 브링업 실행 (완성된 맵 불러오기)
ros2 launch nav2_bringup bringup_launch.py \
    map:=/home/eddy/ros2_ws/maps/slamdunk.yaml \
    autostart:=true \
    use_sim_time:=false

# 8️⃣ArUco 마커 인식 노드 실행 (선택사항)  아직 test 해야 함
ros2 launch aruco_ros aruco_recognition.launch.py
```
 SLAM이 이미 완료된 경우에는 slam_toolbox와 teleop_twist_keyboard는 생략 가능

## 💻 [로컬 PC 환경]

RViz2를 이용해 시각화 및 디버깅을 진행
```
# 1️⃣ Scout 로봇 모델 로드 (URDF)
ros2 launch scout_description scout_base_description.launch.py

# 2️⃣ RViz2 실행 (맵, 로봇, Path, Goal Pose 등 시각화)
ros2 run rviz2 rviz2

# 3️⃣ 웹캠 영상 확인 (rqt_image_view 이용)
3-1. rqt_image_view
3-2. http://192.168.123.250:8080
```

## 📷 웹캠 영상 송출 방식 비교
| 구분                                     | 방식                                    | 장점                                       | 단점                             | CPU 부하 |
| -------------------------------------- | ------------------------------------- | ---------------------------------------- | ------------------------------ | ------ |
| **① rqt_image_view / rviz2 직접 구독**     | `v4l2_camera` 노드의 `/image_raw`를 직접 구독 | ROS 내에서 바로 처리 가능<br>ArUco 마커, SLAM 연동 용이 | 고해상도 시 CPU 부하 큼                | 🔴 높음  |
| **② web_video_server (JPEG_streamer)** | 웹 브라우저로 MJPEG 스트림 송출                  | 가볍고 빠름<br>로컬 네트워크에서 실시간 확인 가능            | 별도 브릿지 노드 필요<br>ROS 내부 처리엔 부적합 | 🟢 낮음  |

### 🗺️ 전체 실행 순서 요약
|  순서 |     실행 위치    | 명령어 요약                              |
| :-: | :----------: | :---------------------------------- |
|  1  | Raspberry Pi | CAN 통신 활성화                          |
|  2  | Raspberry Pi | `scout_base.launch.py` 실행           |
|  3  | Raspberry Pi | `rplidar_a1_launch.py` 실행           |
|  4  | Raspberry Pi | `slam_toolbox` 또는 `nav2_bringup` 실행 |
|  5  | Raspberry Pi | `v4l2_camera_node` (웹캠) 실행          |
|  6  |   Local PC   | `rviz2` 및 `rqt_image_view` 실행       |
