...
# rclpy_component를 위한 import는 필요 없습니다.
...
class AmclResetter(Node):
    ...
    def __init__(self, **kwargs):
        # rclpy.init()이 컴포넌트 컨테이너에 의해 외부에서 처리되므로,
        # Node 생성 시 받은 **kwargs를 super().__init__에 전달해야 합니다.
    ...
    def load_room_coordinates(self):
        ...
        # NOTE: 이 코드는 'scout_robot' 패키지가 빌드되어 'install' 디렉토리에 있을 때 작동합니다.
        try:
            ...
            self.get_logger().error(f"좌표 로딩 중 오류 발생: {e}")
...
# ----------------------------------------------------
def create_node():
    """AmclResetter 컴포넌트 인스턴스를 생성하고 반환합니다."""
    # AmclResetter 클래스 인스턴스 생성 및 반환
    return AmclResetter() 

# 기존 main 함수와 if __name__ == '__main__': 구문은 제거합니다.
# 컴포넌트 컨테이너가 create_node() 함수를 호출하여 노드를 로드합니다.
